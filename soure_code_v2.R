...
merge(TCGA_COAD_expr_fpkm,TCGA_READ_expr_fpkm)->TCGA_CRC_expr_fpkm;
merge(TCGA_COAD_methy_beta,TCGA_READ_methy_beta)->TCGA_CRC_methy_beta;
#------save the data:
...
2^(unique(TCGA_CRC_methy_M[,6]+0.2))/(1+2^(unique(TCGA_CRC_methy_M[,6]+0.2)))->test_beta;
hist(test_beta,breaks=200,prob=T,xaxt='n')->test_beta_hist
axis(side=1,at=seq(0,1,0.1))
lines(density(test_beta),col="red")
#--------------------------mixtools: 
library(mixtools);
unique(TCGA_CRC_methy_M[,6])->test_data
